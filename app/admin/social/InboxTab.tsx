    ...
    <div>
          ...
          {(["open", "resolved", "all"] as const).map((f) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-full text-xs font-medium capitalize transition-colors ${
                filter === f ? "bg-[#1A3A5C] text-white" : "bg-white border border-green-600 text-slate-600"
              }`}
            >
              {f}
            </button>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-3">
          {filtered.map((item) => (
            <div key={item.id} className="bg-white border border-green-600 rounded-xl p-4">
              <div className="flex justify-between items-start mb-2">
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-xs font-medium text-slate-500">{brandName(item.brand_id)}</span>
                  <span className="text-xs px-2 py-0.5 rounded-full bg-slate-100 text-slate-600 capitalize">{item.platform}</span>
                  <span className="text-xs px-2 py-0.5 rounded-full bg-slate-100 text-slate-600 uppercase">{item.source_type}</span>
                  <span
              ...
              {item.auto_reply_text && (
                <p className="text-sm text-green-700 bg-green-100 rounded-lg px-3 py-2 mb-2">
