      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50">
          <div className="bg-white border border-green-600 rounded-2xl p-6 w-full max-w-md">
            ...
            <div className="flex gap-2 mb-4">
              {(["facebook", "instagram", "tiktok", "youtube"] as SocialPlatform[]).map((p) => (
                <button
                  key={p}
...
function ContentCard({
  ...
  async function toggleVideoPreview() {
    ...
    if (!c.video_asset_id) return;
    const selectedAsset = videoAssets.find((v) => v.id === c.video_asset_id);
    const which = selectedAsset?.final_path ? "final" : "raw";
    setVideoPreviewLoading(true);
    setVideoError("");
    try {
      const res = await fetch(`/api/social/video-assets/${c.video_asset_id}/preview-url?which=${which}`);
      const body = await res.json();
      if (!res.ok) throw new Error(body.error ?? `HTTP ${res.status}`);
  ...
  return (
    <div className="bg-white border border-green-600 rounded-xl p-4">
      ...
      {c.platform !== "tiktok" && c.platform !== "youtube" && (c.status === "draft" || c.status === "approved" || c.status === "scheduled") && (
        <div className="mb-3">
      ...
      {(c.platform === "instagram" || c.platform === "tiktok" || c.platform === "youtube") && (c.status === "draft" || c.status === "approved") && (
        <div className="mb-2">
          ...
          <select
            ...
            onChange={(e) => {
              setVideoPreviewUrl(null);
              linkVideoAsset(c.id, e.target.value);
            }}
