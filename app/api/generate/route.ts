...
10. CRITICAL — The Supabase clients already exist (platform-provided) — do NOT write them.
    ...
    Both use \`cookies()\` synchronously (Next 14) and read the tenant schema from \`NEXT_PUBLIC_SUPABASE_SCHEMA\` — you never pass a schema yourself. This means you never write ANY cookie-reading/-writing code yourself either — no \`get\`/\`set\`/\`remove\`/\`getAll\`/\`setAll\` callback, under any variable name, in any file. Confirmed live 2026-09-17: a canary build wrote its own inline \`setAll(cookiesToSet)\` cookie callback in an API route (instead of importing the platform client), and its untyped destructured parameters (\`{ name, value, options }\`) failed the production TypeScript build with \`implicitly has an 'any' type\` — every occurrence of this pattern is both redundant and a guaranteed build failure.
    There are exactly TWO files that construct a Supabase client anywhere in this codebase — \`lib/supabase.ts\` and \`lib/supabase-server.ts\`, both platform-owned, already written for you. Never create a third one under any other name (\`supabase-client.ts\`, \`supabaseClient.ts\`, \`db.ts\`, a per-feature client, etc.) and never call \`createClient\`/\`createBrowserClient\`/\`createServerClient\` yourself anywhere else. Confirmed live 2026-09-17 (same day, a second canary build): a self-invented \`lib/supabase-client.ts\` called \`createClient(supabaseUrl, supabaseKey, ...)\` with the env vars read as plain \`process.env.X\` (typed \`string | undefined\`) instead of asserted (\`process.env.X!\`, exactly like the two real platform files already do) — failed the production TypeScript build with \`Argument of type 'string | undefined' is not assignable to parameter of type 'string'\`. If you ever think you need a new Supabase client file, you are wrong — import one of the two that already exist instead.
...
16. Build-error patterns that have actually broken production builds — check for all of these before emitting a file:
    ...
    - CRITICAL — any component that calls \`useSearchParams()\` MUST be wrapped in \`<Suspense>\` (from \`react\`), or the production build fails outright with \`useSearchParams() should be wrapped in a suspense boundary\` during static generation. This hits confirmation/success/receipt-style pages constantly (e.g. \`/checkout/success?session_id=...\`, \`/book/confirmation?...\`) since they read an ID or status out of the URL after a redirect. Confirmed live 2026-09-17, twice, on exactly this pattern (a storefront checkout-success page, a booking confirmation page). Fix: pull the \`useSearchParams()\` call into its own small child Client Component, then wrap ONLY that child in \`<Suspense fallback={...}>\` from the parent — never call \`useSearchParams()\` directly in a page/layout body without that wrapper.
