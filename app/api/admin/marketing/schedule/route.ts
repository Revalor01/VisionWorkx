...
// Creates a one-off scheduled campaign. Unlike the existing
// /api/admin/marketing/generate + /campaigns pair, this doesn't generate
// content up front — the cron route (app/api/cron/email) generates the
// draft when run_at comes due, so a scheduled campaign always reflects
// whatever's true about the product at send time, not at scheduling time.
export async function POST(req: NextRequest) {
  if (!(await isAdminRequest())) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });

  let body: { product?: MarketingProduct; goal?: string; voiceNotes?: string; runAt?: string; autonomy?: MarketingAutonomy };
  try {
    body = await req.json();
  } catch {
